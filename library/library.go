...
var (
	...
	resolveRegexp = regexp.MustCompile(`^plugin://plugin.video.dainc.*?(\d+)(\W|$)`)
...
func Init() {
	...
	if err := checkMoviesPath(); err != nil {
		xbmc.Notify("dainc", err.Error(), config.AddonIcon())
		return
	}
			...
			case <-timer.C:
				...
				if len(episodes) > 1 {
					...
					if len(labels) > 0 {
						...
						if xbmc.DialogConfirmFocused("dainc", fmt.Sprintf("LOCALIZE[30278];;%s", label)) {
	...
	if updateDelay > 0 {
		if updateDelay < 10 {
			// Give time to dainc to update its cache of libraryMovies, libraryShows and libraryEpisodes
			updateDelay = 10
		}
		go func() {
	...
	go func() {
		...
		if !tmdb.WarmingUp.IsSet() {
			xbmc.Notify("dainc", "LOCALIZE[30147]", config.AddonIcon())
		}
	}()
	...
	if took.Seconds() > 30 {
		xbmc.Notify("dainc", "LOCALIZE[30148]", config.AddonIcon())
	}
...
<movie>
	...
	<uniqueid type="dainc" default="false">%v</uniqueid>
...
func RemoveEpisode(tmdbID int, showID int, seasonNumber int, episodeNumber int) error {
	...
	if !alreadyRemoved {
		...
		return errors.New("Nothing left to remove from dainc")
	}
...
func SyncMoviesList(listID string, updating bool, isUpdateNeeded bool) (err error) {
	...
	if !updating && len(movieIDs) > 0 {
		...
		if config.Get().LibraryUpdate == 0 || (config.Get().LibraryUpdate == 1 && xbmc.DialogConfirmFocused("dainc", fmt.Sprintf("LOCALIZE[30277];;%s", label))) {
...
func AddMovie(tmdbID string, force bool) (*tmdb.Movie, error) {
	...
	if !force && IsDuplicateMovie(tmdbID) {
		xbmc.Notify("dainc", fmt.Sprintf("LOCALIZE[30287];;%s", movie.Title), config.AddonIcon())
...
func AddShow(tmdbID string, force bool) (*tmdb.Show, error) {
	...
	if !force && IsDuplicateShow(tmdbID) {
		xbmc.Notify("dainc", fmt.Sprintf("LOCALIZE[30287];;%s", show.Name), config.AddonIcon())
