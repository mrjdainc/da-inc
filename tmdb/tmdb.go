...
func tmdbCheck(key string) bool {
	...
	if err != nil {
		log.Error(err.Error())
		xbmc.Notify("dainc", "TMDB check failed, check your logs.", config.AddonIcon())
		return false
...
// 				if err != nil {
// 					log.Error(err.Error())
// 					xbmc.Notify("dainc", "Failed listing entities, check your logs.", config.AddonIcon())
// 				} else if resp.Status() != 200 {
// 					message := fmt.Sprintf("Bad status listing entities: %d", resp.Status())
// 					log.Error(message)
// 					xbmc.Notify("dainc", message, config.AddonIcon())
// 				}
