...
const (
	githubUserContentURL    = "https://da-inc.github.io/packages/%s/%s"
	githubAltUserContentURL = "https://dainc.surge.sh/packages/%s/%s"
...
func getAddons(user string, repository string) (*xbmc.AddonList, error) {
	...
	for _, repo := range []string{"plugin.video.dainc", "script.dainc.burst", "context.dainc"} {
...
func GetAddonFiles(ctx *gin.Context) {
	...
	case "addons.xml":
		...
		return
	case "addons.xml.md5":
		// go writeChangelog(user, "plugin.video.dainc")
		// go writeChangelog(user, "script.dainc.burst")
		// go writeChangelog(user, "context.dainc")
