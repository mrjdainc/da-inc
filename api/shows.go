...
import (
	...
	"github.com/mrjdainc/da-inc/providers"
...
func TVLibrary(ctx *gin.Context) {
	shows, err := xbmc.VideoLibraryGetdaincShows()
	...
	for i := (page - 1) * config.Get().ResultsPerPage; i < shows.Limits.Total && i < page*config.Get().ResultsPerPage; i++ {
		...
		if id, err := strconv.Atoi(shows.Shows[i].UniqueIDs.dainc); err == nil {
...
func showSeasonLinks(showID int, seasonNumber int) ([]*bittorrent.TorrentFile, error) {
	...
	if len(searchers) == 0 {
		xbmc.Notify("dainc", "LOCALIZE[30204]", config.AddonIcon())
	}
...
func ShowSeasonLinks(action string, s *bittorrent.Service) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if existingTorrent != nil && (config.Get().SilentStreamStart || xbmc.DialogConfirmFocused("dainc", fmt.Sprintf("LOCALIZE[30608];;[COLOR gold]%s[/COLOR]", existingTorrent.Title()))) {
		...
		if len(torrents) == 0 {
			xbmc.Notify("dainc", "LOCALIZE[30205]", config.AddonIcon())
			return
		}
...
func ShowEpisodeLinks(action string, s *bittorrent.Service) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if existingTorrent != nil && (config.Get().SilentStreamStart || (existingTorrent.IsNextFile && config.Get().SmartEpisodeChoose) || xbmc.DialogConfirmFocused("dainc", fmt.Sprintf("LOCALIZE[30608];;[COLOR gold]%s[/COLOR]", existingTorrent.Title()))) {
			rURL := URLQuery(URLForXBMC(runAction),
				"doresume", doresume,
				"resume", existingTorrent.InfoHash(),
