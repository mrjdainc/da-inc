...
LIBTORRENT_GO = github.com/da-inc/libtorrent-go
...
ifeq ($(TARGET_ARCH), arm)
dist: dainc vendor_$(TARGET_OS) strip checksum
else ifeq ($(TARGET_ARCH), armv6)
dist: dainc vendor_$(TARGET_OS) strip checksum
else ifeq ($(TARGET_ARCH), armv7)
dist: dainc vendor_$(TARGET_OS) strip checksum
...
binaries:
	...
	git clone --depth=1 https://github.com/mrjdainc/da-inc-binaries binaries
