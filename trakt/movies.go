...
func GetMovie(ID string) (movie *Movie) {
	...
	if err := cacheStore.Get(key, &movie); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", fmt.Sprintf("Failed getting Trakt movie (%s), check your logs.", ID), config.AddonIcon())
		}
...
func GetMovieByTMDB(tmdbID string) (movie *Movie) {
	...
	if err := cacheStore.Get(key, &movie); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", "Failed getting Trakt movie using TMDB ID, check your logs.", config.AddonIcon())
			return
		}
...
func Userlists() (lists []*List) {
	...
	if traktUsername == "" {
		xbmc.Notify("dainc", "LOCALIZE[30149]", config.AddonIcon())
		return lists
	}
	...
	if resp.Status() != 200 {
		...
		xbmc.Notify("dainc", errMsg, config.AddonIcon())
		log.Warningf(errMsg)
		return lists
	}
