...
import (
	...
	"github.com/mrjdainc/da-inc/playcount"
...
func GetShow(ID string) (show *Show) {
	...
	if err := cacheStore.Get(key, &show); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", fmt.Sprintf("Failed getting Trakt show (%s), check your logs.", ID), config.AddonIcon())
			return
		}
...
func GetShowByTMDB(tmdbID string) (show *Show) {
	...
	if err := cacheStore.Get(key, &show); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", "Failed getting Trakt show using TMDB ID, check your logs.", config.AddonIcon())
			return
		}
...
func GetShowByTVDB(tvdbID string) (show *Show) {
	...
	if err := cacheStore.Get(key, &show); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", "Failed getting Trakt show using TVDB ID, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&show); err != nil {
			log.Warning(err)
...
func GetEpisodeByID(id string) (episode *Episode) {
	...
	if err := cacheStore.Get(key, &episode); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", "Failed getting Trakt episode, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&episode); err != nil {
			log.Warning(err)
...
func GetEpisodeByTMDB(tmdbID string) (episode *Episode) {
	...
	if err := cacheStore.Get(key, &episode); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", "Failed getting Trakt episode using TMDB ID, check your logs.", config.AddonIcon())
			return
		}
...
func GetEpisodeByTVDB(tvdbID string) (episode *Episode) {
	...
	if err := cacheStore.Get(key, &episode); err != nil {
		...
		if err != nil {
			log.Error(err)
			xbmc.Notify("dainc", "Failed getting Trakt episode using TVDB ID, check your logs.", config.AddonIcon())
			return
		}
		if err := resp.Unmarshal(&episode); err != nil {
			log.Warning(err)
