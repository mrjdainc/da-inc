...
func processLinks(torrentsChan chan *bittorrent.TorrentFile, sortType int, isSilent bool) []*bittorrent.TorrentFile {
	...
	if !isSilent {
		dialogProgressBG = xbmc.NewDialogProgressBG("dainc", "LOCALIZE[30117]", "LOCALIZE[30117]", "LOCALIZE[30118]")
		go func() {
			for {
				select {
				...
				case msg, ok := <-progressUpdate:
					...
					if dialogProgressBG != nil {
						if msg != "skip" {
							dialogProgressBG.Update(progress*100/progressTotal, "dainc", msg)
						}
					} else {
						return
					}
	...
	if !isSilent {
		dialogProgressBG.Update(100, "dainc", "LOCALIZE[30117]")
	}
	...
	// dialogProgressBG.Update(progress*100/progressTotal, "dainc", progressMsg)
	...
	// 	dialogProgressBG.Update(100, "dainc", progressMsg)
