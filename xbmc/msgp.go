...
func (z *UniqueIDs) UnmarshalMsg(bts []byte) (o []byte, err error) {
	...
	for zb0001 > 0 {
		...
		case "dainc":
			z.dainc, bts, err = msgp.ReadStringBytes(bts)
			if err != nil {
				return
			}
		case "Kodi":
...
func (z *UniqueIDs) Msgsize() (s int) {
	s = 1 + 8 + msgp.StringPrefixSize + len(z.Unknown) + 5 + msgp.StringPrefixSize + len(z.TMDB) + 5 + msgp.StringPrefixSize + len(z.TVDB) + 5 + msgp.StringPrefixSize + len(z.IMDB) + 11 + msgp.StringPrefixSize + len(z.TheMovieDB) + 6 + msgp.StringPrefixSize + len(z.Trakt) + 10 + msgp.StringPrefixSize + len(z.dainc) + 5 + msgp.IntSize
	return
}
