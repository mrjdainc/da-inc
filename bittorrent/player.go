...
func (btp *Player) Buffer() error {
	...
	if !btp.t.IsBufferingFinished {
		btp.dialogProgress = xbmc.NewDialogProgress("dainc", "", "", "")
...
func (btp *Player) processMetadata() {
	...
	if btp.p.StoredResume != nil && btp.p.StoredResume.Position > 0 {
		if !config.Get().StoreResume || config.Get().StoreResumeAction == 0 || !(config.Get().SilentStreamStart || config.Get().StoreResumeAction == 2 || xbmc.DialogConfirmFocused("dainc", fmt.Sprintf("LOCALIZE[30535];;%s", btp.p.StoredResume.ToString()))) {
	...
	if status.GetState() == StatusChecking || btp.t.IsRarArchive {
		...
		if btp.t.IsRarArchive && progress >= 100 {
			...
			if err != nil {
				log.Error(err)
				btp.bufferEvents.Broadcast(err)
				xbmc.Notify("dainc", "LOCALIZE[30304]", config.AddonIcon())
				return false, err
			}
			...
			if err != nil {
				log.Error(err)
				btp.bufferEvents.Broadcast(err)
				xbmc.Notify("dainc", "LOCALIZE[30305]", config.AddonIcon())
				return false, err
			}
			...
			if err != nil {
				log.Error(err)
				btp.bufferEvents.Broadcast(err)
				xbmc.Notify("dainc", "LOCALIZE[30306]", config.AddonIcon())
				return false, err
			}
...
func (btp *Player) findExtracted(destPath string) {
	...
	if err != nil {
		log.Error(err)
		btp.bufferEvents.Broadcast(err)
		xbmc.Notify("dainc", "LOCALIZE[30307]", config.AddonIcon())
		return
	}
	if len(files) == 1 {
