...
import (
	...
	"github.com/mrjdainc/da-inc/proxy"
...
func (s *Service) checkAvailableSpace(t *Torrent) bool {
	...
	if availableSpace < sizeLeft {
		...
		xbmc.Notify("dainc", "LOCALIZE[30207]", config.AddonIcon())
...
func (s *Service) AddTorrent(uri string, paused bool) (*Torrent, error) {
	...
	if !s.IsMemoryStorage() && s.config.DownloadPath == "." {
		...
		xbmc.Notify("dainc", "LOCALIZE[30113]", config.AddonIcon())
...
func (s *Service) RemoveTorrent(t *Torrent, forceDrop, forceDelete, isWatched bool) bool {
	...
	if forceDrop || config.Get().KeepDownloading == 2 || len(t.ChosenFiles) == 0 {
		keepDownloading = false
	} else if config.Get().KeepDownloading == 0 || xbmc.DialogConfirm("dainc", fmt.Sprintf("LOCALIZE[30146];;%s", t.Name())) {
	...
	if keepDownloading == false {
		if forceDelete || len(t.ChosenFiles) == 0 {
			...
			deleteAnswer = false
		} else if keepSetting == 2 || xbmc.DialogConfirm("dainc", fmt.Sprintf("LOCALIZE[30269];;%s", t.Name())) {
			deleteAnswer = true
		}
	}
		...
		case <-rotateTicker.C:
			...
			if totalActive > 0 {
				...
				if !s.config.DisableBgProgress && (!s.config.DisableBgProgressPlayback || !s.anyPlayerIsPlaying()) {
					if s.dialogProgressBG == nil {
						s.dialogProgressBG = xbmc.NewDialogProgressBG("dainc", "")
					}
					if s.dialogProgressBG != nil {
						s.dialogProgressBG.Update(showProgress, "dainc", showTorrent)
					}
				}
