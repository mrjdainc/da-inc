...
func Reload() *Configuration {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			xbmc.AddonSettings("plugin.video.dainc")
			xbmc.Dialog("dainc", message)
	...
	info.TempPath = filepath.Join(xbmc.TranslatePath("special://temp"), "dainc")
	...
	if libraryPath == "." {
		...
		panic(settingsWarning)
	} else if strings.Contains(libraryPath, "dainc_library") {
	...
	} else if strings.Contains(torrentsPath, "dainc_torrents") {
	...
	// after dainc's buffer is finished.
...
func CheckBurst() {
	...
	if !Get().SkipBurstSearch && xbmc.DialogConfirmFocused("dainc", "LOCALIZE[30271]") {
		log.Infof("Triggering Kodi to check for script.dainc.burst plugin")
		xbmc.PlayURL("plugin://script.dainc.burst/")
		time.Sleep(15 * time.Second)

		log.Infof("Checking for existence of script.dainc.burst plugin now")
		if xbmc.IsAddonInstalled("script.dainc.burst") {
			xbmc.SetAddonEnabled("script.dainc.burst", true)
			xbmc.Notify("dainc", "LOCALIZE[30272]", AddonIcon())
		} else {
			xbmc.Dialog("dainc", "LOCALIZE[30273]")
		}
	}
}
